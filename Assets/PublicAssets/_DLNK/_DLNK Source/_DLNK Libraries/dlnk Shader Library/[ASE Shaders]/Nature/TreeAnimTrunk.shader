	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float2 texCoord148 = inputMesh.uv1.xy * float2( 1,1 ) + float2( 0,0 );
				float4 lerpResult146 = lerp( float4( 0,0,0,0 ) , appendResult138 , saturate( (_VerticalOffset.x + (texCoord148.y - 0.0) * (_VerticalOffset.y - _VerticalOffset.x) / (1.0 - 0.0)) ));
						...
						)
			{
				...
				float2 texCoord148 = packedInput.uv1.xy * float2( 1,1 ) + float2( 0,0 );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float3 worldNormal15 = normalize( float3(dot(tanToWorld0,tanNormal15), dot(tanToWorld1,tanNormal15), dot(tanToWorld2,tanNormal15)) );
				float smoothstepResult118 = smoothstep( 0.0 , _Smooth , worldNormal15.y);
				float2 texCoord148 = packedInput.ase_texcoord2.zw * float2( 1,1 ) + float2( 0,0 );
				float temp_output_16_0 = saturate( ( (0.0 + (( smoothstepResult118 * _Ammount ) - _Clamp.x) * (1.0 - 0.0) / (_Clamp.y - _Clamp.x)) + (_TopVertical.x + (texCoord148.y - 0.0) * (_TopVertical.y - _TopVertical.x) / (1.0 - 0.0)) ) );
				float4 lerpResult18 = lerp( lerpResult44 , ( _ColorTop * tex2D( _AlbedoTop, (( _UVWorld )?( ( appendResult132 * _TopTiling * 0.1 ) ):( float4( texCoord110, 0.0 , 0.0 ) )).xy ) ) , temp_output_16_0);
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float lerpResult161 = lerp( _MotionValue.y , -_MotionValue.y , cos( _TimeParameters.x * 0.25 ));
				float4 appendResult138 = (float4(( simplePerlin3D165 * lerpResult159 ) , 0.0 , ( simplePerlin3D165 * lerpResult161 ) , 0.0));
				float2 texCoord148 = inputMesh.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				float4 lerpResult146 = lerp( float4( 0,0,0,0 ) , appendResult138 , saturate( (_VerticalOffset.x + (texCoord148.y - 0.0) * (_VerticalOffset.y - _VerticalOffset.x) / (1.0 - 0.0)) ));
		...
		Pass
		{
					...
					)
			{
				...
				float3 worldNormal15 = normalize( float3(dot(tanToWorld0,tanNormal15), dot(tanToWorld1,tanNormal15), dot(tanToWorld2,tanNormal15)) );
				float smoothstepResult118 = smoothstep( 0.0 , _Smooth , worldNormal15.y);
				float2 texCoord148 = packedInput.ase_texcoord3.zw * float2( 1,1 ) + float2( 0,0 );
				float temp_output_16_0 = saturate( ( (0.0 + (( smoothstepResult118 * _Ammount ) - _Clamp.x) * (1.0 - 0.0) / (_Clamp.y - _Clamp.x)) + (_TopVertical.x + (texCoord148.y - 0.0) * (_TopVertical.y - _TopVertical.x) / (1.0 - 0.0)) ) );
				float lerpResult19 = lerp( ( 0.0 * _Glossiness ) , ( 0.0 * _GlossinessTop ) , temp_output_16_0);
...
WireConnection;179;1;11;0
WireConnection;179;7;19;0
WireConnection;179;8;54;0
WireConnection;179;11;146;0
ASEEND*/
//CHKSM=992B774E4490D25FE8DE5F9E726656384CDC3E78